             ...
             <div className='space-y-6'>
              ...
              <div>
                ...
                <ul className='grid grid-cols-1 sm:grid-cols-2 gap-2'>
                  {
                    ["360 Camera", "Bluetooth", "GPS", "Heated Seats", "Rear View Camera"]
                    .map((item) => (
          ...
          className='shadow-lg h-max sticky top-18 rounded-xl p-6 space-y-6 text-gray-500'>
            <p className='flex items-center justify-between text-2xl text-gray-800 font-semibold'>
              ...
              <div className='flex flex-col gap-2'>
                ...
                <input type='date' required id='pickup-date' min={new Date().toISOString().split('T')[0]}
                className='border border-borderColor px-3 py-2 rounded-lg'/>
              </div>

              <div className='flex flex-col gap-2'>
                <label htmlFor='return-date'>Return Date</label>
                <input type='date' required id='pickup-date'
                className='border border-borderColor px-3 py-2 rounded-lg'/>
              </div>

              <button className='w-full bg-primary hover:bg-primary-dull py-3 transition all 
